...
def og_image():
    ...
    # small tile-grid motif on the right, echoing the app icon
    grid_size = 340
    gx0, gy0 = W - grid_size - 90, (H - grid_size) // 2
    gap = grid_size * 0.09
    cell = (grid_size - 2 * gap) / 3
    for row in range(3):
        for col in range(3):
            x0 = gx0 + col * (cell + gap)
            y0 = gy0 + row * (cell + gap)
            if row == 1 and col == 1:
                ...
                fill = TILE
            d.rounded_rectangle([x0, y0, x0 + cell, y0 + cell], radius=int(cell * 0.22), fill=fill)
    ...
    title_font = load_font(["fonts/fraunces-latin-700-normal.woff2"], 64)
    # PIL can't read woff2 directly; fall back to a bundled system TTF if available
    import glob
    ttf_candidates = glob.glob("/usr/share/fonts/**/*.ttf", recursive=True)
    serif_candidates = [f for f in ttf_candidates if 'serif' in f.lower() or 'georgia' in f.lower()] or ttf_candidates
    sans_candidates = [f for f in ttf_candidates if 'dejavu' in f.lower() or 'sans' in f.lower()] or ttf_candidates
    title_font = load_font(serif_candidates[:3], 66)
    subtitle_font = load_font(sans_candidates[:3], 32)
    tag_font = load_font(sans_candidates[:3], 26)

    pad = 80
    d.text((pad, 210), "Bingo în predicare", font=title_font, fill=TILE)
    d.text((pad, 300), "Milwaukee 2026", font=subtitle_font, fill=GOLD)
    d.text((pad, 350), "Joc pentru serviciul de teren  ·  funcționează offline", font=tag_font, fill=(159, 184, 196, 255))

    img.save("icons/og-image.png")
